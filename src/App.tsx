...
const generateAnalysisId = (url: string): string => {
  const timestamp = Date.now();
  return `${url.replace(/^https?:\/\//, '').replace(/[^a-zA-Z0-9]/g, '-')}-${timestamp}`;
};
...
function App() {
  ...
  const handleAnalyze = async (formData: FormData) => {
    ...
    const initialAnalysis: WebsiteAnalysis = {
      ...
      url: formData.url,
      timestamp,
      patientExperience: { doctorSearch: false, videoConsultation: false, onlinePayment: false, chatbot: false, helpline: false, score: 0 },
      digitalPresence: { googleReviews: { rating: null, count: null }, socialMedia: { instagram: false, facebook: false, youtube: false }, score: 0 },
      websitePerformance: { loadTime: null, responsive: false, lighthouse: null, score: 0 },
      overallScore: 0,
      loading: true,
      error: null,
    };
    ...
    try {
      ...
      const updatedAnalysis: WebsiteAnalysis = {
        ...
        websitePerformance: {
          loadTime,
          responsive: responseData.lighthouse?.viewport ? true : false,
          lighthouse: responseData.lighthouse,
          score: 0,
        },
        loading: false,
      };
      ...
      setAnalysis({ ...initialAnalysis, loading: false, error: 'Failed to analyze website. Please check the URL and try again.' });
    }
  };
  ...
  return showResults ? (
    <ResultsPage analysis={analysis} onNewAnalysis={handleNewAnalysis} onShare={handleShare} />
  ) : (
    <LandingPage onAnalyze={handleAnalyze} />
  );
}
